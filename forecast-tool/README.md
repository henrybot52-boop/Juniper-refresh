...
| **Change Orders** | A change-order log per project (#, description, status, Δ contract value, Δ cost budget, Δ contingency, date, notes). Statuses Pending / Approved / Executed / Rejected — **only Executed change orders flow into the forecast**, adding their deltas to the NTE cap, cost budget and contingency used everywhere. Pipeline KPIs (executed / approved / pending), CSV import (merge or replace) and export, and a Change Orders sheet in the workbook. |
